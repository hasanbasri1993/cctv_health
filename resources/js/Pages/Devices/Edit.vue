    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-3">
                <Link :href="`/devices/${device.id}`" class="rounded-md p-1.5 text-slate-500 transition-colors hover:bg-slate-800/60 hover:text-slate-200">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                    </svg>
                </Link>
                <div>
                    <h2 class="text-[17px] font-semibold text-slate-100">Edit: {{ device.name }}</h2>
                    <p class="text-[13px] text-slate-500">{{ device.ip_address }}:{{ device.port }}</p>
                </div>
            </div>
        </template>
            ...
            <div class="rounded-lg border border-white/[0.06] bg-slate-900/60 shadow-[0_1px_2px_rgba(0,0,0,0.4)]">
                <form @submit.prevent="submit" class="divide-y divide-white/[0.06]">
                    <div class="space-y-5 p-6">
                        ...
                        <!-- Password -->
                        <div>
                            <label for="password" class="block text-[13px] font-medium text-slate-300">
                            ...
                            <input
                                ...
                                placeholder="••••••••"
                                class="mt-1.5 block w-full rounded-md border border-white/[0.08] bg-slate-800/60 px-3 py-2 text-[13px] text-slate-100 placeholder-slate-600 focus:border-cyan-500/50 focus:outline-none focus:ring-1 focus:ring-cyan-500/30"
                            />
